...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "CargaUy-ear.ear");

        // Current directory is the root of the "CargaUy-web" project. Go up one level
        // and enter the "CargaUy-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../CargaUy-ejb/target/CargaUy-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "CargaUy-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/CargaUy-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "CargaUy-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
